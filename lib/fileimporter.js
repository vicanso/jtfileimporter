...
(function() {
  var FileImporter, JTMerge, events, fs, path, _,
  ...
  FileImporter = (function(_super) {
    ...
    */
    ...
    function FileImporter(options) {
      ...
      if (this.options.merge) {
        this.jtMerge = new JTMerge(this.options.merge);
      }
      if (this.hosts) {
    ...
    FileImporter.prototype.addTemplateFiles = function(template) {
      var templateFilesInfo, _ref;
      templateFilesInfo = (_ref = FileImporter.TemplateInfos) != null ? _ref[template] : void 0;
      if (templateFilesInfo) {
        this.importJs(templateFilesInfo.jsList);
        return this.importCss(templateFilesInfo.cssList);
      }
    };
    ...
    */
    ...
    FileImporter.prototype._getExportFilesHTML = function(type, merge) {
      ...
      if (this.options.debug && type !== 'css') {
        resultFiles = _.map(resultFiles, function(file) {
          return "/src" + file;
        });
      }
      if (this.jtMerge && !this.options.debug) {
        console.dir('..........');
        resultFiles = this.jtMerge.getMergeExportFiles(resultFiles);
      }
    ...
    */
    ...
    FileImporter.prototype._getUrl = function(file) {
      var crc32, host, hosts, index, urlPrefix, version;
      hosts = this.hosts;
      version = this.options.version;
      if (this.options.crc32List) {
        crc32 = this.options.crc32List[file];
        if (crc32) {
          version = crc32;
        }
      }
